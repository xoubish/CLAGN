"""Fixed 2x600-second H-beta continuum sensitivity; local research only.

Uses the official NGPS ETC, 2x2 bins and a 1-arcsec central slice. Archival
continuum normalization and explicit sky scenarios are not a 2026 forecast.
"""
...
def one(target):
    ...
    signature=hashlib.sha256(json.dumps(base,sort_keys=True).encode()+b'central-2x600-v1').hexdigest()
