...
def get_macleod16_sample(coords, labels, *, verbose=1):
    ...
    None
        The input lists are modified in place.
    """
    # Journal HTML scraping blocked (403); use NED bibcode query instead
    macleod_CSQ = Ned_query_refcode_exceptions('2016MNRAS.457..389M')
    if macleod_CSQ is None:
        return

    for ra, dec in zip(macleod_CSQ['RA'], macleod_CSQ['DEC']):
        coords.append(SkyCoord(ra, dec, frame='icrs', unit='deg'))
...
def get_macleod19_sample(coords, labels, *, verbose=1):
    ...
    macleod19_CSQ = table2[(table2['CLQ?'] > 0) & (table2['Nsigma'] > 3)]
...
def get_graham_sample(coords, labels, *, verbose=1):
    ...
    """
    # Journal HTML scraping blocked (403); use NED bibcode query instead
    CSQ = Ned_query_refcode_exceptions('2020MNRAS.491.4925G')
    if CSQ is None:
        return

    for ra, dec in zip(CSQ['RA'], CSQ['DEC']):
        coords.append(SkyCoord(ra, dec, frame='icrs', unit='deg'))
