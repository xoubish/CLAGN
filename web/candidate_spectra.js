...
function spectralColor(i) { return `hsl(${(210+i*137.508)%360} 58% 40%)`; }
...
function specPanel(t) {
  ...
  let svg=`<svg viewBox="0 0 ${W} ${H}" width="100%" role="img" aria-label="${selected.length} selected spectra of ${esc(t.name)}"><defs><clipPath id="spectral-clip"><rect x="${L}" y="${T}" width="${W-L-R}" height="${H-B-T}"/></clipPath></defs>`;
  ...
  svg+='<g clip-path="url(#spectral-clip)">';
...
function redrawSpectra() {
  const t=D.targets.find(t=>t.name===state.selected), panel=$('spectrum-panel');
  if(t&&panel)panel.innerHTML=specPanel(t);
}
document.addEventListener('change',e=>{
  const box=e.target.closest('[data-spectrum-index]');if(!box)return;
  const t=D.targets.find(t=>t.name===state.selected);if(!t)return;
  ...
  spectralSelection.set(t.name,chosen);redrawSpectra();
});
