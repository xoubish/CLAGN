...
objects,dobjects,flabels,zlist,keeps = unify_lc_gp_parallel(df_lc,redshifts,bands_inlc=bands_inlc,xres=160)
objects2,dobjects2,flabels2,zlist2,keeps2 = unify_lc(df_lc,redshifts,bands_inlc=bands_inlc,xres=160)
...
#mapper2 = umap.UMAP(n_neighbors=100,min_dist=0.99,metric='manhattan',random_state=10).fit(data)
...
plt.tight_layout()
plt.savefig('output/umap-ztfw.png')
```
...
# Assuming 'mapper.embedding_' is your data and 'labc' is your dictionary of labels to indices
hist, x_edges, y_edges = np.histogram2d(mapper.embedding_[:, 0], mapper.embedding_[:, 1], bins=16)
plt.figure(figsize=(13, 8))

# Define groups of labels
group_labels = {
    'SDSS_QSO': ['SDSS_QSO'],
    'WISE_Variable': ['WISE_Variable'],
    'Optical_Variable': ['Optical_Variable'],
...
for group_name, labels in group_labels.items():
    ...
    plt.subplot(2, 3, i)  # Adjust the subplot layout as needed
    plt.title(group_name)
    plt.contourf(x_edges[:-1], y_edges[:-1], prob.T, levels=16, alpha=0.8, cmap=custom_cmap)
    plt.colorbar()
    plt.axis('off')
    i += 1
...
```{code-cell}
mapper = umap.UMAP(n_neighbors=1000,min_dist=0.999,metric=dtw_distance,random_state=30).fit(data)
#mapper = umap.UMAP(n_neighbors=50,min_dist=0.9,metric='manhattan',random_state=25).fit(clean_data)
